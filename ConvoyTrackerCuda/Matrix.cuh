  ...
  public:
    ...
    protected:
      ...
      ) const
      {
        ...
        unsigned column = 0;
        while ( ZERO == matrix[ (row * columns) + column] )
          ++column;
      ...
      )
      {
        for ( unsigned column = 0; column < columns; ++column )
          matrix[ (row * columns) + column] /= divisor;
      }
      ...
      {
        for ( unsigned column = 0; column < columns; ++column )
          matrix[ (row * columns) + column] += matrix[ (addRow * columns) + column] * scale;
      }
      ...
      )
      {
        ...
        // Setup matrix sizes.
	matrix = (TYPE *)malloc(sizeof(TYPE) * (rowNumber * columnNumber));
   	if (matrix == NULL) {
    	...
    	/*for (int i = 0; i < rowNumber; ++i) {
    	    matrix[i] = (TYPE *)malloc(sizeof(TYPE) * columnNumber);
    	    if (matrix[i] == NULL) {
    	        perror("memory allocation failure");
    	        exit(EXIT_FAILURE);
    	    }
    ...
    public:
      ...
      ) const
      {
        ...
        for ( unsigned row = 0; row < rows; ++row )
          for ( unsigned column = 0; column < columns; ++column )
            lower.matrix[(row * columns) + column] = ZERO;

        for ( unsigned row = 0; row < rows; ++row )
        {
          TYPE value = upper.matrix[(row * columns) + row];
          if ( ZERO != value )
          {
            upper.divideRow( row, value );
            lower.matrix[(row * columns) + row] = value;
          }

          for ( unsigned subRow = row + 1; subRow < rows; ++subRow )
          {
            TYPE value = upper.matrix[(subRow * columns) + row];
            upper.rowOperation( subRow, row, -value );
            lower.matrix[(subRow * columns) + row] = value;
          }
        }
      }
      ...
      )
      {
        ...
        for ( unsigned row = startRow; row <= endRow; ++row )
        {
          ...
          for ( unsigned column = startColumn; column <= endColumn; ++column )
            subMatrix.matrix[ (row - startRow)*(endColumn - startColumn + 1) +(column - startColumn)] =
              matrix[ (subRow * columns) + column];
        }
      ...
      void reducedRowEcholon()
      {
        ...
        for ( unsigned rowIndex = 0; rowIndex < rows; ++rowIndex )
        {
          ...
          TYPE divisor = matrix[(row * columns) + column];
          if ( ZERO != divisor )
          {
            ...
            for ( unsigned subRowIndex = ( rowIndex + 1 ); subRowIndex < rows; ++subRowIndex )
            {
              ...
              if ( ZERO != matrix[(subRow * columns) + column] )
                rowOperation
                (
                  subRow,
                  row,
        ...
        for ( unsigned row = 0; row < rows; ++row )
          for ( unsigned column = 0; column < columns; ++column )
          {
            result +=
              ...
              * otherMatrix.matrix[(row * columns) + column];
          }
      ...
      Matrix const getTranspose() const
      {
        ...
        for ( unsigned row = 0; row < rows; ++row )
          for ( unsigned column = 0; column < columns; ++column )
            result.matrix[(column * columns) + row] = matrix[(row * columns) + column];
      ...
      ) const
      {
        ...
        for ( unsigned row = 0; row < rows; ++row )
          for ( unsigned column = 0; column < columns; ++column )
            result.matrix[(row * columns) + column] =
              matrix[(row * columns) + column]
              + otherMatrix.matrix[(row * columns) + column];
      ...
      ) const
      {
        ...
        for ( unsigned row = 0; row < rows; ++row )
          ...
          {
            result.matrix[(row * otherMatrix.columns) + column] = ZERO;

            for ( unsigned index = 0; index < columns; ++index )
              result.matrix[(row * otherMatrix.columns) + column] +=
                matrix[(row * columns) + index]
                * otherMatrix.matrix[(index * otherMatrix.columns) + column];
          }
      ...
      ) const
      {
        ...
        for ( unsigned row = 0; row < rows; ++row )
          for ( unsigned column = 0; column < columns; ++column )
            result.matrix[(row * columns) + column] = matrix[(row * columns) + column] * scalar;
      ...
      )
      {
        ...
        for ( unsigned row = 0; row < rows; ++row )
          for ( unsigned column = 0; column < columns; ++column )
            matrix[(row * columns) + column] = data[ index++ ];
      ...
      ) const
      {
        ...
        for ( unsigned row = 0; row < rows; ++row )
          for ( unsigned column = 0; column < columns; ++column )
            if ( matrix[(row * columns) + column] != value.matrix[(row * columns) + column] )
              isEqual = false;
        ...
        columns( columnsParameter )
      {
        ...
        for ( unsigned row = 0; row < rows; ++row )
        {
          ...
          for ( unsigned column = 0; column < columns; ++column )
            matrix[(row * columns) + column] = ZERO;
        }
      }
      ...
      )
      {
        ...
        for ( unsigned row = 0; row < rows; ++row )
        {
          ...
          for ( unsigned column = 0; column < columns; ++column )
          {
            ...
            matrix[(row * columns) + column] = copyMatrix.matrix[ (rowIndex * columns) + columnIndex];
      ...
      )
      {
        ...
        for ( unsigned row = 0; row < copyMatrixA.rows; ++row )
          for ( unsigned column = 0; column < copyMatrixA.columns; ++column )
            matrix[(row * columns) + column] = copyMatrixA.matrix[(row * copyMatrixA.columns) + column];

        for ( unsigned row = 0; row < copyMatrixB.rows; ++row )
          for ( unsigned column = 0; column < copyMatrixB.columns; ++column )
            matrix[ (row + rowOffset)*columns + (column + columnOffset) ] =
              copyMatrixB.matrix[(row * copyMatrixB.columns) + column];
      }
        ...
        Matrix< TYPE >( rowsParameter, columnsParameter )
      {
        ...
        for ( unsigned row = 0; row < Matrix< TYPE >::rows; ++row )
        {
          for ( unsigned column = 0; column < Matrix< TYPE >::columns; ++column )
            if ( row == column )
              Matrix< TYPE >::matrix[(row * Matrix< TYPE >::columns) + column] = ONE;
            else
              Matrix< TYPE >::matrix[(row * Matrix< TYPE >::columns) + column] = ZERO;
        }
      }
  };
