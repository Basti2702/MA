...
DataReader::DataReader() {
	...
	//allocate host memory and create device pointer
	...
	error = cudaHostAlloc((void**) &dist, NUMBER_LASERRAYS*sizeof(float), cudaHostAllocMapped);
	if (error != cudaSuccess) {
		printf("cudaGetDevice returned error %s (code %d), line(%d)\n",
				cudaGetErrorString(error), error, __LINE__);
	}
	...
	error = cudaHostAlloc((void**) &thresh, NUMBER_LASERRAYS*sizeof(float), cudaHostAllocMapped);
	if (error != cudaSuccess) {
		printf("cudaGetDevice returned error %s (code %d), line(%d)\n",
				cudaGetErrorString(error), error, __LINE__);
	}
...
/*
 * writes the three relevant laserpoints in outputarray for each segment
 */
...
/*
 * computes the threshold between the given points
 */
...
/*
 * transforms one position from polar to cartesian coordinates
 */
...
/*
 * distribute work to threads to transform the positions
 */
...
/*
 * kernel for distance computation
 */
...
int DataReader::processLaserData(std::string number, double currentSpeed, double currentYawRate, PointCellDevice* h_vehicles)
{
	...
	//compute distances and thresholds in parallel
